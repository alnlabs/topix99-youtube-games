...
class YTChat {
  constructor(videoId) {
    ...
    this._retryCount = 0; // Track retry attempts
    this._lastErrorLog = 0; // Throttle error logging
    this._chatUnavailable = false; // Track if chat is permanently unavailable
  }
  ...
  async _launch() {
    ...
    try {
      ...
      if (!chatLoaded) {
        // Throttle error logging to avoid spam (log once per 5 minutes)
        const now = Date.now();
        if (!this._chatUnavailable || now - this._lastErrorLog > 300000) {
          console.warn("[ytchat] Chat container not found - live chat may not be available for this video. Game will continue without chat.");
          this._lastErrorLog = now;
          this._chatUnavailable = true;
        }

        // Schedule retry with exponential backoff (but don't spam logs)
        if (!this.isStopping) {
          if (this.restartTimer) clearTimeout(this.restartTimer);
          const retryDelay = Math.min(30000 * Math.pow(2, this._retryCount || 0), 300000);
          this._retryCount = (this._retryCount || 0) + 1;

          // Only log retry info on first few attempts or every 10th attempt
          if (this._retryCount <= 3 || this._retryCount % 10 === 0) {
            console.log(`[ytchat] Will retry chat connection in ${retryDelay / 1000}s (attempt ${this._retryCount})`);
          }

          this.restartTimer = setTimeout(() => {
            this._launch(); // Retry launch
          }, retryDelay);
        }
        return; // Exit gracefully instead of throwing
      }

      // Chat loaded successfully - reset flags
      if (this._chatUnavailable) {
        console.log("[ytchat] Chat connection restored!");
        this._chatUnavailable = false;
      }
      this._retryCount = 0; // Reset retry count on successful launch
      ...
      // Throttle error logging to avoid spam (log once per 5 minutes)
      const now = Date.now();
      if (now - this._lastErrorLog > 300000) {
    ...
    if (this.page) {
      try {
        ...
        if (!chatLoaded) {
          ...
          const now = Date.now();
          if (!this._chatUnavailable || now - this._lastErrorLog > 300000) {
            console.warn("[ytchat] Chat container not found during restart - live chat may not be available. Will retry.");
            this._lastErrorLog = now;
            this._chatUnavailable = true;
          }

          // Close the new page since it didn't work
        ...
        this._retryCount = 0; // Reset retry count on successful restart
